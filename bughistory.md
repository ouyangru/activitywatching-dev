...
## 2026-09-07T00:12:44+08:00 · Upload Queue Wedged by Sleep-Spanning Windows (HTTP 422)

- **问题：** 采集器日志出现 `upload FAILED, backing off 300s, remaining=3511` 且 remaining 持续增长。服务端返回 HTTP 422，心跳正常但事件数据从 14:03 起全部滞留本地（最终积压 3514 条）。手动复现批次确认：FastAPI 校验拒绝 `duration_ms ≤ 300000`，队列头部两条记录分别为 3,641,047ms（1 小时）和 8,742,945ms（2.4 小时）。
- **根因：** `emit_window()` 用系统时钟差计算窗口时长，电脑睡眠/恢复后第一笔采样把整个睡眠期算进单一窗口（window_start 冻结在睡前）。超限记录按 FIFO 卡在批次头部，整批被 422 拒绝后按网络故障退避重试同一批，形成毒丸：后端永久拒绝 + 采集器无限重试 = 队列永久堵塞。上午两次睡眠（10:35、11:36 附近）各产生一条毒丸。
- **解决：** v0.2.4 三层修复：① 源头钳制——`emit_window()` 将 duration_ms 钳制到 [1, 300000]（睡眠跨窗时长被截断，配套的大 idle_ms 仍如实告知服务端用户不在）；② 防御截断——`serialize()` 对 process(≤260)/window_title(≤2048)/clipboard_events(≤100) 镜像后端 schema 上限，杜绝同类毒丸；③ 毒丸免疫——`flush()` 区分传输失败与 HTTP 状态码，遇到 4xx（除 408/429）丢弃该批并写日志（`server rejected batch with HTTP xxx; dropping N event(s)`），网络类失败才走退避，队列永不被永久性拒绝卡死。存量毒丸用脚本钳制清洗（备份 queue.jsonl.bak-poison）。
- **版本信息：** activity_collector 0.2.3 → 0.2.4（MinGW 14.2 / CMake 3.31）；后端约束见 backend/app/schemas.py FeatureWindow。
- **验证：** 清洗 + v0.2.4 启动后 3514 条积压约 3 分钟内排空（remaining→0）；服务端 windows-AOSIKA last_seen 距查询 19 秒、window_count 7474→10997、collector_version=0.2.4；之后 persist/heartbeat 正常滚动。附注：调试期间发现从 AI 会话沙箱派生启动的实例无法读写 %LOCALAPPDATA% 状态文件（仅网络可用），最终经 schtasks（任务计划程序，独立进程树）启动获得干净实例；该问题属工具环境，与采集器代码无关。

## 2026-09-07T00:26:08+08:00 · 全天时间轴重复图例与过期设备无法移除

- **问题现象：** 全天时间轴按设备横向拆成多张卡片且重复显示完整颜色图例，不符合从综合到单设备逐行比较的阅读顺序；超过 48 小时的设备仍可能出现在服务端列表，用户也无法主动移除演示设备。
- **根因：** 时间轴复用了单设备图表函数，每张图独立创建坐标系和图例；48 小时判断仅依赖浏览器中的事件时间，没有使用服务器接收/心跳时间，也没有持久化的设备隐藏状态。
- **解决方案：** 时间轴合并为一张 00:00—24:00 共享坐标图，纵向依次排列所有设备综合与各设备；活动颜色标签提升为时间分布面板的全局图例；服务器按最后接收或心跳时间过滤超过 48 小时未上报设备；新增删除设备接口与界面按钮，删除仅隐藏设备并保留历史，新事件或心跳会自动恢复显示。
- **版本信息：** 基于 2546125，首页静态资源版本 20260907-2。
- **验证结果：** 本地回归 66 passed；前端语法与 diff 检查通过；模拟 DOM/ECharts 验证共享时间轴的综合优先行序、单一全局颜色标签、无重复图例及目的标签通过；API 测试覆盖 48 小时过滤、手动删除、历史保留、重新上报恢复和不存在设备 404。
