...
## 2026-09-07T00:48:22+08:00 · 时间轴视觉臃肿与首页纠正未进入 Agent 记忆

- **问题现象：** 全天时间轴设备名过长、画布过宽且相邻同类状态被切成大量细条；具体分项始终挤在图中；首页虽能改分类，但请求没有携带 remember，提示却声称会用于后续修正，空闲状态仍需进入日报处理。
- **根因：** 时间轴直接逐段绘制且为设备全名预留 175px；首页修改固定发送 `{category}`，没有区分应用语义纠正与空闲时段纠正，也没有调用既有的 Agent 记忆和线下活动接口。
- **解决方案：** 设备标签压缩为综合、电脑/手机加短名称，时间轴最大宽度收紧到 920px、左侧留白缩至 92px、行高降低；同类别且间隔不超过 2 分钟的连续片段合并，具体行为仅在悬停提示中展开；首页“其他”通过片段纠正接口按应用记忆，“空闲/无设备记录”通过线下活动接口按时间段确认，并提供默认勾选的“让 Agent 参考”。
- **版本信息：** 基于 9e0b208，首页静态资源版本 20260907-3。
- **验证结果：** 本地回归 66 passed；前端语法与 diff 检查通过；模拟 ECharts 验证设备短名、92px 紧凑坐标区、同类片段合并和悬停分项内容通过。

## 2026-09-07T01:11:51+08:00 · 时间轴设备平台丢失与色块间距偏大

- **问题现象：** 时间轴三行设备名都显示为“综合”，无法区分 Windows 与 Android；坐标绘图区距离边框仍偏远，设备行距和活动色块厚度显得臃肿；部分相同状态未能合并。
- **根因：** 分布请求组装结果时只返回 label、category、purpose、timeline，丢失 device_id 和 platform，短名称函数因此把每行误判为综合；坐标区和色块沿用较宽参数，连续状态合并间隔仅 2 分钟。
- **解决方案：** 保留设备平台和标识字段，行名改为综合、Windows/Android 加短设备名；坐标区左右边距收紧到 76px/6px，色块高度由行高 48% 降至 32%，单行预算由 42px 降至 34px；相同类别连续片段的合并间隔放宽至 5 分钟。
- **版本信息：** 基于 8b30963，首页静态资源版本 20260907-4。
- **验证结果：** 本地回归 66 passed；前端语法与 diff 检查通过；模拟 ECharts 验证平台字段保留、Windows/Android 行名、5 分钟合并边界、76px/6px 坐标边距及 32% 色块高度通过。

## 2026-09-07T01:25:33+08:00 · 综合时间轴无法直接纠正其他与空闲状态

- **问题现象：** 用户必须在设备明细或日报中寻找原始片段，无法直接点击综合时间轴里的“其他/空闲”色块进行纠正并让 Agent 参考。
- **根因：** ECharts 综合行只提供悬停提示，没有点击交互；综合片段是派生结果，没有直接暴露可修改的原始片段标识。
- **解决方案：** 为综合行的其他/空闲色块增加可点击边框、指针和提示，点击后打开快速纠正窗口；其他状态按时间重叠关系定位各设备原始片段并批量调用片段纠正接口，空闲状态按色块时间范围调用线下活动接口；两类纠正都可选择写入 Agent 参考记忆，保存后刷新综合图。
- **版本信息：** 基于 fb422eb，首页静态资源版本 20260907-5。
- **验证结果：** 本地回归 66 passed；前端语法与 diff 检查通过；模拟点击综合“其他”色块可定位底层片段并携带 remember 保存；模拟点击综合“空闲”色块可按准确 UTC 区间保存为睡眠并写入 Agent 时段参考。

## 2026-09-07T14:13:46+08:00 · Android 重传不恢复且日报忽略同时设备

- **问题现象：** 被删除或超过 48 小时隐藏的 Android 设备重新上传后仍不显示；综合状态和 Agent 日报多数沿用 Windows 主活动，没有说明同一时段 Android 正在进行的活动。
- **根因：** 重传批次若全是已有序号，`INSERT OR IGNORE` 不更新事件的 `received_at`，隐藏标记虽被清除却随即再次被 48 小时规则过滤；综合算法把其他设备放入 `secondary`，日报提示词只读取主活动，并在同分时用 Windows 作为稳定裁决项。
- **解决方案：** 新增独立的设备上报回执，每次批次到达即刷新，不受事件是否重复影响，并以此参与 48 小时判断；日报提示词同时传入每个综合片段的主活动和 Android/其他次要设备活动，要求结合分析但不重复累计时长，同时更新提示版本使旧日报缓存自动失效。
- **版本信息：** 基于 1e30747，日报提示版本 `cross-device-secondary-v1`。
- **验证结果：** 本地回归 68 passed；新增测试验证 Android 全重复批次可恢复设备显示，以及日报提示包含同一时段的 Android 状态；既有跨设备主活动、48 小时隐藏和统计不重复测试全部通过。

## 2026-09-11T18:25:04+08:00 · 一键发布脚本被 Windows 换行破坏

- **问题现象：** 在 WSL/Linux 中直接执行 `scripts/deploy-aliyun.sh` 时出现 `$'\r': command not found` 和 `set: pipefail: invalid option name`，一键发布无法继续。
- **根因：** 发布脚本被提交为 CRLF 换行；Linux Bash 把每行末尾的回车符当成命令内容，导致解释器选项无法识别。
- **解决方案：** 新增 `.gitattributes`，强制所有 `.sh` 文件以 LF 存储，并重新规范化发布脚本，保证 Windows 工作区提交后仍能被 Linux Bash 直接执行。
- **版本信息：** 基于 2763029。
- **验证结果：** `bash -n scripts/deploy-aliyun.sh` 语法检查通过；完整一键发布流程成功执行，线上提交、主服务状态与健康接口均核验正常。


### 2026-09-09T08:04:27+00:00 空闲时段无法补录学习或工作
- 现象：线下活动接口拒绝学习/工作，人工补录统一归入生活事务。
- 根因：OfflineActivityRequest 类别白名单过窄，apply_offline 将 purpose 写死；扩大分类后还会把线下学习归入原空闲应用排行。
- 解决：允许完整人工分类，按分类确定目的，并从应用排行排除人工线下标注；保留时间切分、原始记录和撤销机制。
- 版本：基于 83fe08d，分支 codex/frontend-history-reflection。
- 验证：tests.test_offline_corrections_unit 的 4 项测试通过，覆盖部分时段、目的映射、撤销、原始活动优先和时间参数校验。


### 2026-09-09T08:05:24+00:00 日报目的统计与历史加载一致性
- 现象：日报目的占比原先使用逐设备 insights 累计，在跨设备重叠时与去重后的总时长不一致；旧日期请求也可能晚到并覆盖新选择。
- 根因：分类和目的图表使用不同统计口径，日报异步读取没有请求代次检查。
- 解决：目的图改用现有去重 summary 接口；历史总览、日报和对比通过请求代次拒绝旧结果，加载失败隐藏过期内容，提供独立重试入口。
- 版本：基于 83fe08d，分支 codex/frontend-history-reflection，前置提交 aa8e6bf / 7c15298。
- 验证：6 项 JavaScript 逻辑测试通过，包括乱序响应、错误状态和历史参数；Python 4 项修正测试通过。完整 API 与浏览器验收尚未完成，限制见 FRONTEND.md。


### 2026-09-09T12:11:13+00:00 总览页专注洞察面板与时间线错位且大量留白
- 现象：1150px 以下视口时间线独占整行、洞察面板被挤到下方并拉伸出约 250px 空白；指标卡仅 75px 高、应用排行每行 15px，面板头重脚轻；同排时两面板高度也不一致（650px 对 677px）。
- 根因：styles.css 的 1150px 断点将 timeline-panel 设为 grid-column:1/-1；design.css 在 1050px 就把 trio 布局折叠为单列；洞察面板内容不足以填满与时间线面板等高的拉伸区域，且未设固定高度导致两面板高度不齐。
- 解决：删除 1150px 整行规则，堆叠断点统一移至 850px；洞察面板与时间线面板同为 650px 固定高度、排行列表内部滚动；新增行为排行区块（复用 insights.behaviors 数据）充实面板，指标卡与排行行加高；850px 以下恢复自然高度。静态资源版本号统一提升避免旧缓存。
- 版本：基于 cb278b1，main 分支，静态资源 v=20260909-4。
- 验证：node --check 4 个 JS 文件通过；node --test 前端 6 项测试全过；Playwright 实测 1440px 与 1000px 视口两面板 y 坐标与高度完全一致（650px），无内容裁剪，行为排行 4 项正常渲染；850px 以下单列布局不受影响。


### 2026-09-09T22:50:17+08:00 Agent 上游失败引发重复请求与日志风暴
- 现象：生产日志 30 分钟内出现 195 轮增强和 588 次 DeepSeek 402 失败，失败候选随采集批次反复提交，并产生 18,942 行日志。
- 根因：OpenAI 兼容客户端吞掉上游错误后只返回空值，调度器无法区分暂时无结果和明确的认证、余额或限流错误；失败结果不落证据缓存，下一批采集会立即重试。
- 解决：客户端增加线程安全的指数退避熔断；401/402/403 直接暂停一小时，429 参考 Retry-After，其余失败从 60 秒指数退避；Agent 分类和日报在冷却期不再排队或继续分批调用，规则底账照常提供结果；状态接口公开剩余冷却秒数。
- 版本：基于 86f1c97，后端版本 0.4.2。
- 验证：新增 402 连续调用测试，确认两次业务调用只产生一次网络请求且冷却超过 3500 秒；Agent/API 定向测试 45 passed，全量回归 76 passed。


### 2026-09-09T22:50:17+08:00 API 缺少耗时观测且生产 Agent 正文日志过量
- 现象：Uvicorn 与 Nginx 访问日志均关闭，无法从日志判断具体慢接口；生产环境默认输出完整脱敏 prompt，造成日志膨胀并暴露不必要的活动上下文。
- 根因：HTTP 中间件只添加隐私响应头，没有计时与结构化请求日志；Agent 正文日志开关默认开启且不区分开发和生产环境。
- 解决：为所有 API 记录方法、纯路径、状态码、后端耗时和响应大小，默认超过 200ms 提升为 WARNING；响应增加 Server-Timing 与 X-Response-Time-Ms；日志不记录查询参数；生产环境默认关闭 Agent prompt 正文，仍允许显式开启。
- 版本：基于 86f1c97，后端版本 0.4.2。
- 验证：新增测试确认耗时响应头存在、日志含状态码且不含查询令牌；生产默认正文关闭和显式覆盖测试通过；全量回归 76 passed。


### 2026-09-09T22:50:17+08:00 大型 JSON 响应未压缩放大公网加载延迟
- 现象：线上单日时间线约 272KB、综合时间线约 207KB，首页会并发请求多份大型 JSON，在高延迟公网链路上加载缓慢。
- 根因：应用与现有 Nginx 配置均未对 application/json 响应实际启用压缩。
- 解决：后端为大于 1KB 的响应启用 gzip，压缩级别为 5，小响应保持原样以避免无收益的 CPU 开销。
- 版本：基于 86f1c97，后端版本 0.4.2。
- 验证：新增大型时间线响应测试，确认客户端声明接受 gzip 时返回 Content-Encoding: gzip 且解压后的 JSON 完整；全量回归 76 passed。


### 2026-09-09T23:18:55+08:00 Agent 无效结构化结果随采集批次重复调用
- 现象：首次部署 0.4.2 后，线上日志显示 DeepSeek 已成功响应，但有 3 条候选连续出现“解析成功 3/3、new=0”，并随每 10 秒采集批次再次调用。
- 根因：模型返回了 JSON 数组，但条目缺少必填字段或未通过类别校验；代码只缓存有效 evidence，没有为缺失和无效条目设置重试间隔。
- 解决：为模型遗漏、空响应、非法 JSON 和字段校验失败的 digest 增加一小时进程内冷却；有效条目仍立即落证据表，冷却只影响对应无效候选，不改变规则底账或其他 Agent 判断。
- 版本：基于 60f67ca，后端版本 0.4.2。
- 验证：上线日志复现无效条目连续重试；新增测试确认连续两轮 enrich 只调用模型一次；Agent 定向测试 20 passed，全量回归 77 passed。


### 2026-09-10T11:55:00+08:00 刷新页面停留在历史日期而非今天
- 现象：用户翻看历史日期后刷新浏览器，页面仍停留在上次查看的历史日期，而不是回到今天。
- 根因：`syncDay()` 用 history.replaceState 把 `?day=YYYY-MM-DD` 写进地址栏，刷新时该参数被 `initialDay()` 再次读取；对比页同理持久化 `start`/`end` 参数，刷新后仍加载旧区间。
